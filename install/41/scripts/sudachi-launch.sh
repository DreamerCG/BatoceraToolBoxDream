...
# LAUNCH ROM
ulimit -H -n 819200; ulimit -S -n 819200; ulimit -S -n 819200 sudachi;
ulimit -H -n 819200; ulimit -S -n 819200; ulimit -S -n 819200 sudachi.AppImage;
ulimit -H -n 819200; ulimit -S -n 819200; ulimit -S -n 819200 AppRun.wrapped;
rom="$(echo "$@" | sed 's,-f -g ,,g')"
if [[ "$rom" = "" ]]; then
	# if no rom then launch emulator
	/userdata/system/switch/sudachi/sudachi.AppImage -f -g > >(tee "$log1") 2> >(tee "$log2" >&2)
else
	...
	# Run ROM
	/userdata/system/switch/sudachi/sudachi.AppImage -f -g "$rom" 1>"$log1" 2>"$log2"
fi
