...
log1=/userdata/system/switch/logs/yuzu-out.txt 2>/dev/null
log2=/userdata/system/switch/logs/yuzu-err.txt 2>/dev/null
rm $log1 2>/dev/null && rm $log2 2>/dev/null

# LAUNCH ROM
ulimit -H -n 819200; ulimit -S -n 819200; ulimit -S -n 819200 yuzu;
ulimit -H -n 819200; ulimit -S -n 819200; ulimit -S -n 819200 yuzu.AppImage;
rom="$(echo "$@" | sed 's,-f -g ,,g')"
if [[ "$rom" = "" ]]; then
	# if no rom then launch emulator
	/userdata/system/switch/emulateur/yuzu.AppImage -f -g > >(tee "$log1") 2> >(tee "$log2" >&2)
else
	...
	# Run ROM
	/userdata/system/switch/emulateur/yuzu.AppImage -f -g "$rom" 1>"$log1" 2>"$log2"
fi
