...
def list_sdl_gamepads(sdlversion):
    ...
    os.environ["SDL_JOYSTICK_HIDAPI_XBOX_ONE"] = "0"   #no gyro , don't care
        ...
        if not system.isOptSet('yuzu_auto_controller_config') or system.config['yuzu_auto_controller_config'] != "0":
            ...
            for nplayer, pad in enumerate(playersControllers, start=0):
                ...
                for x in yuzuButtonsMapping:
                    yuzuConfig.set("Controls", player_nb_str + "_" + x, '"{}"'.format(YuzuMainlineGenerator.setButton(emulator, yuzuButtonsMapping[x], pad.guid, pad.inputs, guid_port[pad.guid])))
                for x in yuzuAxisMapping:
                    yuzuConfig.set("Controls", player_nb_str + "_" + x, '"{}"'.format(YuzuMainlineGenerator.setAxis(yuzuAxisMapping[x], pad.guid, pad.inputs, guid_port[pad.guid])))
