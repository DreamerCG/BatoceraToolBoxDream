...
if [[ "$id" != "" ]] && [[ "$id" != "0" ]]; then
	...
	genline=$(cat /userdata/system/switch/configgen/generators/yuzuGenerator.py | grep 'inputguid = controller.guid')
	replace="$genline"
	replaced=$(echo "$genline" | sed 's,^.*= ,,g')
	old=$(cat /userdata/system/switch/configgen/generators/yuzuGenerator.py | grep 'inputguid = "' | head -n1 | sed 's,^.* = ,,g')
	new=\"$id\"
	with="                inputguid = \"$id\""
	...
	if [[ "$replace" != "$with" ]]; then 
		...
		sed -i "s/^.*inputguid = controller.guid/                inputguid = \"$id\"/g" /userdata/system/switch/configgen/generators/yuzuGenerator.py
