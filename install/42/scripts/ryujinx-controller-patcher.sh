...
if [[ "$getavaid" != "" ]] && [[ "$getavaid" != "0" ]]; then
	...
	avaid=""$(echo $getavaid)""
	genline=$(cat /userdata/system/switch/configgen/generators/ryujinxGenerator.py | grep "cvalue\['id")
	replace="$genline"
	replaced=$(echo "$genline" | sed 's,^.*= ,,g')
	old=$(cat /userdata/system/switch/configgen/generators/ryujinxGenerator.py | grep "cvalue\['id" | sed 's,^.* = ,,g')
	new=\"$avaid\"
	...
	if [[ "$replace" != "$with" ]]; then 
		...
		sed -i "s/^.*cvalue\['id'\] =.*$/                cvalue['id'] = \"$getavaid\"/g" /userdata/system/switch/configgen/generators/ryujinxGenerator.py
