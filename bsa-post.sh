...
post_install_ryujinx() {
	...
	copy_make_executable "ryujinx_config.sh.key" "$switch_install_roms_ports_dir" "$switch_ports_dir"
...
post_install_yuzu_common() {
	...
	copy_make_executable "yuzu_config.sh.key" "$switch_install_roms_ports_dir" "$switch_ports_dir"
